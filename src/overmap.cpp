                   ...
                   input_context *inp_ctxt, const draw_data_t &data)
{
    ...
    if( z == 0 && uistate.overmap_show_railroad_station_labels ) {
        draw_railroad_station_labels( w, tripoint( cursx, cursy, z ) );
    }
    ...
    if( inp_ctxt != nullptr ) {
        ...
        print_hint( "TOGGLE_RAILROAD_STATION_LABELS" );
...
void overmap::draw_railroad_station_labels( WINDOW *w, const tripoint &center )
{
    const int win_x_max = getmaxx( w );
    const int win_y_max = getmaxy( w );
    const int sm_radius = std::max( win_x_max, win_y_max );
    ...
    const point screen_center_pos( win_x_max / 2, win_y_max / 2 );
    for( const auto &element : overmap_buffer.get_railroad_stations_near( omt_to_sm_copy( center ), sm_radius ) ) {
        ...
        mvwprintz( w, text_y, text_x_min, i_green, "%s", element.city->name.c_str() );
    }
}
...
tripoint overmap::draw_overmap(const tripoint &orig, const draw_data_t &data)
{
    ...
    ictxt.register_action("TOGGLE_RAILROAD_STATION_LABELS");
    ...
    do {
        ...
        } else if( action == "TOGGLE_RAILROAD_STATION_LABELS" ) {
            uistate.overmap_show_railroad_station_labels = !uistate.overmap_show_railroad_station_labels;
...
void overmap::place_railroad_stations()
{
    ...
    while ( railroad_stations.size() < size_t( NUM_RAILROAD_STATIONS ) ) {
        ...
        if ( ter( cx, cy, 0 ) == settings.default_oter ){
             ...
             ter( cx + 3, cy + 1, 0 ) = oter_id( "road_end_west" );
             ter( cx + 1, cy - 1, 0 ) = oter_id( "railroad_ns" );
             ter( cx + 1, cy - 3, 0 ) = oter_id( "railroad_ns" );
             ter( cx + 0, cy + 0, 0 ) = oter_id( "railroad_station_1_3_north" );
             ter( cx + 1, cy + 0, 0 ) = oter_id( "railroad_station_1_2_north" );
             ter( cx + 2, cy + 0, 0 ) = oter_id( "railroad_station_1_1_north" );
             ter( cx + 0, cy + 1, 0 ) = oter_id( "railroad_station_1_6_north" );
             ter( cx + 1, cy + 1, 0 ) = oter_id( "railroad_station_1_5_north" );
             ter( cx + 2, cy + 1, 0 ) = oter_id( "railroad_station_1_4_north" );
             ter( cx + 0, cy + 2, 0 ) = oter_id( "railroad_station_1_9_north" );
             ter( cx + 1, cy + 2, 0 ) = oter_id( "railroad_station_1_8_north" );
             ter( cx + 2, cy + 2, 0 ) = oter_id( "railroad_station_1_7_north" );

             city tmp1;
             tmp1.x = cx + 1;
             tmp1.y = cy - 1;
             tmp1.s = railroad_station_size;
             railroad_stations.push_back( tmp1 );

             city tmp2;
             tmp2.x = cx + 1;
             tmp2.y = cy - 3;
             tmp2.s = railroad_station_size;
             railroad_stations.push_back( tmp2 );
        }
    }
}
