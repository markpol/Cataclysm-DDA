...
{
    return x >= 0 && x < MAPLIMIT_X && y >= 0 && y < MAPLIMIT_Y;
}

void scent_map::shift( const int sm_shift_x, const int sm_shift_y )
{
    scent_array<int> new_scent;
    for( size_t x = 0; x < MAPLIMIT_X; ++x ) {
        for( size_t y = 0; y < MAPLIMIT_Y; ++y ) {
...
bool scent_map::inbounds( const tripoint &p ) const
{
    ...
    return p.x >= 0 && p.x < MAPLIMIT_X && p.y >= 0 && p.y < MAPLIMIT_Y &&
...
void scent_map::update( const tripoint &center, map &m )
{
    ...
    // The code I'm modifying used [MAPLIMIT_X]. I'm staying with that to avoid new bugs.
    ...
    // MAPLIMIT_X, but if that changes, this may need tweaking.
