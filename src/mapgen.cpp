...
ssssssssssssssssssssssss\n",
        ...
        if (density > 1) {
            place_spawns( GROUP_ZOMBIE, 2, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, density );
        } else {
            add_spawn(mon_zombie, rng(0, 5), SUBMAP_MAX_X, SUBMAP_MAX_Y);
        }
        ...
        if (zlevel == 0) { // We're on ground level
            ...
            for (int i = 0; i <= SUBMAP_MAX_X; i++) {
                for (int j = 0; j <= SUBMAP_MAX_Y; j++) {
                    ...
                    if( ( i == 0 && is_lab( t_east ) ) || i == SUBMAP_MAX_X ) {
                        if (ter(i, j) == t_sewage) {
                            ter_set(i, j, t_bars);
                    ...
                    } else if( ( j == 0 && is_lab( t_north ) ) || j == SUBMAP_MAX_Y ) {
                        if (ter(i, j) == t_sewage) {
                            ter_set(i, j, t_bars);
...
|--------|-..-|--------|\n",
                ...
                const auto range = points_in_rectangle( { lw, tw, abs_sub.z }, { SUBMAP_MAX_X - rw, SUBMAP_MAX_Y - bw, abs_sub.z } );
                ...
                case 1: // Cross shaped
                    ...
                    if (one_in(2)) {
                        ...
                        science_room(this, SEEX + 2, tw, SUBMAP_MAX_X - rw, SEEY - 3, zlevel, 3);
                    } else {
                        ter_set(SEEX + int(SEEX / 2), SEEY - 2, t_door_metal_c);
                        science_room(this, SEEX + 2, tw, SUBMAP_MAX_X - rw, SEEY - 3, zlevel, 2);
                    }
                    // Bottom left
                    if (one_in(2)) {
                        ter_set(int(SEEX / 2), SEEY + 1, t_door_metal_c);
                        science_room(this, lw, SEEY + 2, SEEX - 3, SUBMAP_MAX_Y - bw, zlevel, 0);
                    } else {
                        ter_set(SEEX - 2, SEEY + int(SEEY / 2), t_door_metal_c);
                        science_room(this, lw, SEEY + 2, SEEX - 3, SUBMAP_MAX_Y - bw, zlevel, 1);
                    }
                    // Bottom right
                    if (one_in(2)) {
                        ter_set(SEEX + int(SEEX / 2), SEEY + 1, t_door_metal_c);
                        science_room(this, SEEX + 2, SEEY + 2, SUBMAP_MAX_X - rw, SUBMAP_MAX_Y - bw,
                                     zlevel, 0);
                    } else {
                    ...
                    if (rw == 1) {
                        ter_set(SUBMAP_MAX_X, SEEY - 1, t_door_metal_c);
                        ter_set(SUBMAP_MAX_X, SEEY    , t_door_metal_c);
                    }
                    if (bw == 1) {
                        ter_set(SEEX - 1, SUBMAP_MAX_Y, t_door_metal_c);
                        ter_set(SEEX    , SUBMAP_MAX_Y, t_door_metal_c);
                    }
                    if (terrain_type == "lab_stairs" || terrain_type == "ice_lab_stairs") { // Stairs going down
                        ...
                        if (rw != 1) {
                            stair_points.push_back(point(SUBMAP_MAX_X - 2, SEEY - 1));
                            stair_points.push_back(point(SUBMAP_MAX_X - 2, SEEY - 1));
                ...
                case 2: // tic-tac-toe # layout
                    ...
                    science_room(this, SEEX + 4, tw, SUBMAP_MAX_X - rw, SEEY - 5, zlevel, rng(2, 3));
                    science_room(this, lw, SEEY - 3, SEEX - 5, SEEY + 2, zlevel, 1);
                    science_room(this, SEEX + 4, SEEY - 3, SUBMAP_MAX_X - rw, SEEY + 2, zlevel, 3);
                    science_room(this, lw, SEEY + 4, SEEX - 5, SUBMAP_MAX_Y - bw, zlevel, rng(0, 1));
                    science_room(this, SEEX - 3, SEEY + 4, SEEX + 2, SUBMAP_MAX_Y - bw, zlevel, 0);
                    science_room(this, SEEX + 4, SEEX + 4, SUBMAP_MAX_X - rw, SUBMAP_MAX_Y - bw,
                                 zlevel, 3 * rng(0, 1));
                    if (rw == 1) {
                        ter_set(SUBMAP_MAX_X, SEEY - 1, t_door_metal_c);
                        ter_set(SUBMAP_MAX_X, SEEY    , t_door_metal_c);
                ...
                case 3: // Big room
                    ...
                    science_room(this, lw, tw, SUBMAP_MAX_X - rw, SUBMAP_MAX_Y - bw,
                                 zlevel, rng(0, 3));
                    if (t_above == "lab_stairs" || t_above == "ice_lab_stairs") {
                        if( const auto p = random_point( points_in_rectangle( { lw, tw, abs_sub.z }, { SUBMAP_MAX_X - rw, SUBMAP_MAX_Y - bw, abs_sub.z } ), [this]( const tripoint &n ) { return ter( n ) == t_rock_floor; } ) ) {
                            ter_set( *p, t_stairs_up );
                        }
                    }
                    if (rw == 1) {
...
-----------++----------|\n",
                        ...
                        place_spawns( GROUP_LAB_ALIEN_1, 3, 12, 12, 12, 12, density );
                        place_spawns( GROUP_LAB_ALIEN_2, 3, 2, 2, 2, 2, density );
                        place_spawns( GROUP_LAB_ALIEN_3, 3, 2, 7, 2, 7, density );
                        place_spawns( GROUP_LAB_ZOMBIE_SCIENTIST, 2, 12, 18, 12, 18, density );
                        for (int i = 0; i <= 23; i++) {
                            for (int j = 0; j <= 23; j++) {
                                if (this->furn(i, j) == f_counter) {
...
-----------++----------|\n",
        ...
        case 1:
            ...
            place_spawns( GROUP_ROBOT_SECUBOT, 1, 6, 6, 6, 6, density );
            place_spawns( GROUP_ROBOT_SECUBOT, 1, SUBMAP_MAX_X - 6, 6, SUBMAP_MAX_X - 6, 6, density );
            place_spawns( GROUP_ROBOT_SECUBOT, 1, 6, SUBMAP_MAX_Y - 6, 6, SUBMAP_MAX_Y - 6, density );
            place_spawns( GROUP_ROBOT_SECUBOT, 1, SUBMAP_MAX_X - 6, SUBMAP_MAX_Y - 6, SUBMAP_MAX_X - 6, SUBMAP_MAX_Y - 6, density );
                ...
                for (int i = lw; i <= rw; i++) {
                    for (int j = tw; j <= bw; j++) {
                        if (j == tw || j == bw) {
                            ...
                            place_spawns( GROUP_NETHER, 1, i, j, i, j, density );
                        }
                    }
                }
            }
        ...
        line(this, t_chainfence_h,            0,            0, SUBMAP_MAX_X,            0);
        line(this, t_chaingate_l,            11,            0,           12,            0);
        line(this, t_chainfence_h,            0, SUBMAP_MAX_Y, SUBMAP_MAX_X, SUBMAP_MAX_Y);
        line(this, t_chaingate_l,            11, SUBMAP_MAX_Y,           12, SUBMAP_MAX_Y);
        line(this, t_chainfence_v,            0,            0,            0, SUBMAP_MAX_X);
        line(this, t_chaingate_l,             0,           11,            0,           12);
        line(this, t_chainfence_v, SUBMAP_MAX_X,            0, SUBMAP_MAX_X, SUBMAP_MAX_Y);
        line(this, t_chaingate_l,  SUBMAP_MAX_X,           11, SUBMAP_MAX_X,           12);
        ...
        if (zlevel == 0) { // Ground floor
            ...
            square(this, t_rock_floor, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y);
            ...
            case 2: // Spreading water
                ...
                place_spawns( GROUP_SEWER, 1, 4, 4, 4, 4, density );

                square(this, t_water_dp, SUBMAP_MAX_X - 4, 4, SUBMAP_MAX_X - 3, 6);
                place_spawns( GROUP_SEWER, 1, SUBMAP_MAX_X - 4, 4, SUBMAP_MAX_X - 4, 4, density );
            ...
            case 3: { // Flipping walls puzzle
                ...
                line(this, t_rock, SEEX + 2, 0, SUBMAP_MAX_X, 0);
            ...
            if (t_above == "temple_stairs") {
                line(this, t_stairs_up, SEEX, SUBMAP_MAX_Y, SEEX + 1, SUBMAP_MAX_Y);
            }
        ...
        square(this, t_rock_floor, SEEX, 5, SEEX + 1, SUBMAP_MAX_Y);
        line(this, t_stairs_up, SEEX, SUBMAP_MAX_Y, SEEX + 1, SUBMAP_MAX_Y);
        ...
        int ladderx = rng(0, SUBMAP_MAX_X), laddery = rng(0, SUBMAP_MAX_Y);
        while( !dat.is_groundcover( ter(ladderx, laddery) ) ) {
            ladderx = rng(0, SUBMAP_MAX_X);
            laddery = rng(0, SUBMAP_MAX_Y);
        }
        ...
        if (t_above == "mine_shaft") { // We need the entrance room
            ...
            case 5: { // Dark worm!
                ...
                for (int i = 0; i < num_worms; i++) {
                    ...
                    if (sides.empty()) {
                        place_spawns( GROUP_DARK_WYRM_ONLY, 1, 0, 0, SEEX, SEEY, density );
                        i = num_worms;
                    } else {
                        point p;
                        ...
                        place_spawns( GROUP_DARK_WYRM_ONLY, 1, p.x, p.y, p.x, p.y, density );
                    }
                }
            }
            break;
        ...
        if (terrain_type == "mine_down") { // Don't forget to build a slope down!
            ...
            if (open.empty()) { // We'll have to build it in the center
                ...
                case NORTH:
                    ...
                    break;
                case EAST:
                    square(this, t_rock_floor, SEEX + 1, SEEY - 3, SUBMAP_MAX_X - 6, SEEY + 2);
        ...
        if (t_east  == "mine") {
            square(this, t_rock_floor, SUBMAP_MAX_X - 3, SEEY, SUBMAP_MAX_X, SEEY + 1);
        } else {
            face.push_back(EAST);
        }
        ...
        case 2: { // The Thing dog
            ...
            place_spawns( GROUP_DOG_THING_ONLY, 1, rng(SEEX, SEEX + 1), rng(SEEY, SEEY + 1), rng(SEEX, SEEX + 1), rng(SEEY, SEEY + 1), density );
        ...
        case 4: { // Amigara fault
            ...
            case NORTH:
                square(this, t_rock, 0, 0, SUBMAP_MAX_X, 4);
                line(this, t_fault, 4, 4, SUBMAP_MAX_X - 4, 4);
                break;
            case EAST:
 ...
 fF|-D-|XXXXXXX-      s_\n\
   ...
   |www|_______fFFFFFFFF\n\
        ...
        if (x_in_y(1, 2)) {
            place_spawns( GROUP_ROBOT_HAZMATBOT, 1, 10, 10, 5, 5, density );
        }
 ...
 f    |!!!!!!!!!%..r| |^\n",
            ...
            if (x_in_y(1, 2)) {
                place_spawns( GROUP_ROBOT_HAZMATBOT, 1, 22, 12, 22, 12, density );
            }
            if (x_in_y(1, 2)) {
                place_spawns( GROUP_ROBOT_HAZMATBOT, 1, 23, 18, 23, 18, density );
            }
            //lazy radiation mapping
            for (int x = 0; x <= 23; x++) {
                for (int y = 0; y <= 23; y++) {
...
.x6x..|S.T|l.|^.ddd.| f \n",
            ...
            if (x_in_y(1, 2)) {
                place_spawns( GROUP_ROBOT_HAZMATBOT, 1, 7, 10, 7, 10, density );
            }
            if (x_in_y(1, 2)) {
                place_spawns( GROUP_ROBOT_HAZMATBOT, 1, 11, 16, 11, 16, density );
            }
            //lazy radiation mapping
            for (int x = 0; x <= 23; x++) {
                for (int y = 0; y <= 23; y++) {
...
$$$$-|-|=HH-|-HHHH-|####\n",
    ...
    if (terrain_type == "megastore_entrance") {
        ...
        line(this, t_wall_glass, 0, 0, SUBMAP_MAX_X, 0);
        ...
        square(this, t_floor, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y);

        // Randomly pick contents
        switch (rng(1, 5)) {
        case 1: { // Groceries
            bool fridge = false;
            for (int x = rng(2, 3); x < SUBMAP_MAX_X; x += 3) {
        ...
        case 3: // Clothing
            ...
            for (int y = 13; y <= SUBMAP_MAX_Y - 1; y += 3) {
                ...
                if (one_in(3)) {
                    ...
                    place_items("shoes",      75, 2, y, SUBMAP_MAX_X - 2, y, false, 0);
                } else if (one_in(2)) {
                    place_items("bags",       75, 2, y, SUBMAP_MAX_X - 2, y, false, 0);
                } else {
                    place_items("allclothes", 75, 2, y, SUBMAP_MAX_X - 2, y, false, 0);
                }
            }
            break;
        ...
        // Add some spawns
        for (int i = 0; i < 15; i++) {
            int x = rng(0, SUBMAP_MAX_X), y = rng(0, SUBMAP_MAX_Y);
            if (ter(x, y) == t_floor) {
        ...
        if (t_north != "megastore_entrance" && t_north != "megastore") {
            line(this, t_wall, 0, 0, SUBMAP_MAX_X, 0);
        }
        if (t_east != "megastore_entrance" && t_east != "megastore") {
            line(this, t_wall, SUBMAP_MAX_X, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y);
        }
        ...
        if (t_west != "megastore_entrance" && t_west != "megastore") {
            line(this, t_wall, 0, 0, 0, SUBMAP_MAX_Y);
        }
        ...
        // Front wall
        line(this, t_wall,  0, 10,  SUBMAP_MAX_X, 10);
        line(this, t_door_locked, SEEX - 1, 10, SEEX, 10);
        int winx1 = rng(2, 4);
        int winx2 = rng(4, 6);
        line(this, t_window, winx1, 10, winx2, 10);
        line(this, t_window, SUBMAP_MAX_X - winx1, 10, SUBMAP_MAX_X - winx2, 10);
        winx1 = rng(7, 10);
        winx2 = rng(10, 11);
        line(this, t_window, winx1, 10, winx2, 10);
        line(this, t_window, SUBMAP_MAX_X - winx1, 10, SUBMAP_MAX_X - winx2, 10);
        ...
        build_mansion_room(this, room_mansion_courtyard, 0, 0, SUBMAP_MAX_X, 9, dat);
        square(this, t_floor, 0, 11, SUBMAP_MAX_X, SUBMAP_MAX_Y);
        build_mansion_room(this, room_mansion_entry, 0, 11, SUBMAP_MAX_X, SUBMAP_MAX_Y, dat);
        ...
        if (t_west != "mansion_entrance" && t_west != "mansion") {
            ...
            for (int i = 1; i < SEEY; i++) {
                if (passable(1, i) && passable(1, SUBMAP_MAX_Y - i)) {
                    if (consecutive == 3) {
                        consecutive = 0;    // No really long windows
                    } else {
                        consecutive++;
                        ter_set(0, i, t_window);
                        ter_set(0, SUBMAP_MAX_Y - i, t_window);
                    }
                } else {
                    consecutive = 0;
                }
            ...
            for (int i = 1; i < SEEX; i++) {
                if (passable(i, SUBMAP_MAX_Y - 1) &&
                    ...
                    if (consecutive == 3) {
                        ...
                        consecutive++;
                        ter_set(i, SUBMAP_MAX_Y, t_window);
                        ter_set(SUBMAP_MAX_X - i, SUBMAP_MAX_Y, t_window);
                    }
                } else {
                    consecutive = 0;
                }
        ...
        if (t_east != "mansion_entrance" && t_east != "mansion") {
            int consecutive = 0;
            for (int i = 1; i < SEEY; i++) {
                if (passable(SUBMAP_MAX_X - 1, i) &&
                    passable(SUBMAP_MAX_X - 1, SUBMAP_MAX_Y - i)) {
                    if (consecutive == 3) {
                        consecutive = 0;    // No really long windows
                    } else {
                        consecutive++;
                        ter_set(SUBMAP_MAX_X, i, t_window);
                        ter_set(SUBMAP_MAX_X, SUBMAP_MAX_Y - i, t_window);
                    }
                } else {
                    consecutive = 0;
                }
        ...
        if (t_north != "mansion_entrance" && t_north != "mansion") {
            int consecutive = 0;
            for (int i = 1; i < SEEX; i++) {
                if (passable(i, 1) && passable(SUBMAP_MAX_X - i, 1)) {
                    if (consecutive == 3) {
                        consecutive = 0;    // No really long windows
                    } else {
                        consecutive++;
                        ter_set(i, 0, t_window);
                        ter_set(SUBMAP_MAX_X - i, 0, t_window);
                    }
                } else {
                    consecutive = 0;
                }
        ...
        if (t_above == "slimepit_down") {
            ...
            case 2:
                ...
                break;
            case 3:
                ter_set(SUBMAP_MAX_X + 1 -rng(1, 3), rng(0, 2), t_slope_up);
                break;
            case 4:
                ter_set(SUBMAP_MAX_X + 1 -rng(1, 3), SEEY * 2 - rng(1, 3), t_slope_up);
            }
        }

        place_spawns( GROUP_GOO, 1, 0, 0, SEEX, SEEY, density );
        place_items("sewer", 40, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
        ...
        do {
            ...
            if (step > 2) { // First couple of chambers are safe
                ...
                if (monrng <= 24) {
                    place_spawns( GROUP_TRIFFID_INNER, 1, spawnx, spawny, spawnx, spawny, density );
                } else {
                    ...
                    place_spawns( GROUP_SPIDER, 1, spawnx, spawny, spawnx, spawny, density );
                }
            }
        ...
        do {
            ...
            if (chance >= 10) { // Add a spawn
                place_spawns( GROUP_TRIFFID_INNER, 10, x, y, x, y, density );
            }
        ...
        place_spawns( GROUP_TRIFFID_HEART_ONLY, 1, 21, 21, 21, 21, density ); //@todo: single spawn
    } else {
    ...
    if (is_ot_type("subway", terrain_type)) { // FUUUUU it's IF ELIF ELIF ELIF's mini-me =[
        ...
        if (is_ot_type("sewer", t_east) &&
            ...
            if (connects_to(t_east, 3)) {
                ...
                ter_set(SUBMAP_MAX_X - 3, SEEY, t_door_metal_c);
                ter_set(SUBMAP_MAX_X - 3, SEEY - 1, t_door_metal_c);
            }
        }
...
void map::rotate(int turns)
{
    ...
    for( const std::shared_ptr<npc> &i : npcs ) {
        ...
        switch( turns ) {
            case 3:
                new_x = old_y;
                new_y = SUBMAP_MAX_X - old_x;
                break;
            case 2:
                new_x = SUBMAP_MAX_X - old_x;
                new_y = SUBMAP_MAX_Y - old_y;
                break;
            case 1:
                new_x = SUBMAP_MAX_Y - old_y;
                new_y = old_x;
                break;
        }
...
void science_room(map *m, int x1, int y1, int x2, int y2, int z, int rotate)
{
    ...
    case room_vivisect:
        ...
        m->place_spawns( GROUP_CYBORG_BROKEN, 10, int(((x1 + x2) / 2)+1), int(((y1 + y2) / 2)+1),int(((x1 + x2) / 2)+1), int(((y1 + y2) / 2)+1), density );
        break;
...
void silo_rooms(map *m)
{
    ...
    do {
        ...
        if (one_in(2)) { // True = top/bottom, False = left/right
            ...
            width  = rng(2, 5);
            height = 2;
            if (x + width >= SUBMAP_MAX_X) {
                width = SUBMAP_MAX_X - 1 - x;    // Make sure our room isn't too wide
            }
        } else {
            x = rng(0, 4);
...
void mx_helicopter(map &m, const tripoint &abs_sub)
{
    ...
    m.place_items("helicopter", 20, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
    ...
    m.place_spawns( GROUP_MAYBE_MIL, 2, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, density );//0.1f = 1-5
...
void mx_military(map &m, const tripoint &)
{
    ...
    for (int i = 0; i < num_monsters; i++) {
        ...
        m.place_spawns( GROUP_NETHER_PORTAL_EASY, 1, mx, my, mx, my, density );
    }
    m.place_spawns( GROUP_MAYBE_MIL, 2, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y,
                 density);//0.1f = 1-5
    m.place_items("rare", 25, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
}
...
void mx_roadblock(map &m, const tripoint &abs_sub)
{
    ...
    if (mil) { //Military doesn't joke around with their barricades!
        line(&m, t_fence_barbed, SUBMAP_MAX_X, 4, SUBMAP_MAX_X, 10);
        line(&m, t_fence_barbed, SUBMAP_MAX_X - 2, 13, SUBMAP_MAX_X - 2, 19);
        ...
        if (one_in(3)) {  // Chicken delivery
            ...
            m.place_spawns( GROUP_ROBOT_CHICKENBOT_ONLY, 1, 0, 0, SEEX, SEEY, density );
        ...
        line_furn(&m, f_barricade_road, SUBMAP_MAX_X, 4, SUBMAP_MAX_X, 10);
        line_furn(&m, f_barricade_road, SUBMAP_MAX_X - 2, 13, SUBMAP_MAX_X - 2, 19);
...
void mx_drugdeal(map &m, const tripoint &abs_sub)
{
    ...
    for (int i = 0; i < num_bodies_b; i++) {
        ...
        do { // Loop until we find a valid spot to dump a body, or we give up
            if (north_south) {
                x = rng(0, SUBMAP_MAX_X);
                y = rng(SEEY + 3, SUBMAP_MAX_Y);
                x_offset = 0;
                y_offset = 1;
            } else {
                x = rng(SEEX + 3, SUBMAP_MAX_X);
                y = rng(0, SUBMAP_MAX_Y);
                x_offset = 1;
                y_offset = 0;
            }
...
void mx_portal(map &m, const tripoint &abs_sub)
{
    ...
    for (int i = 0; i < num_monsters; i++) {
        ...
        m.place_spawns( GROUP_NETHER_PORTAL, 1, mx, my, mx, my, density );
    }
}
...
void mx_crater(map &m, const tripoint &abs_sub)
{
    ...
    int x = rng(size, SUBMAP_MAX_X - size), y = rng(size, SUBMAP_MAX_Y - size);
...
void mx_fumarole(map &m, const tripoint &)
{
    int x1 = rng(0,    SEEX     - 1), y1 = rng(0,    SEEY     - 1),
        x2 = rng(SEEX, SUBMAP_MAX_X), y2 = rng(SEEY, SUBMAP_MAX_Y);
    ...
    for (int i = x - 5; i <= x + 5; i++) {
        for (int j = y - 5; j <= y + 5; j++) {
            if (rng(1, 9) >= trig_dist(x, y, i, j)) {
                ...
                if (one_in(15)) {
                    m.place_spawns( GROUP_NETHER, 1, i, j, i, j, density );
                }
            }
        }
    }
    ...
    case ARTPROP_BREATHING:
        for (int i = cx - 1; i <= cx + 1; i++) {
            for (int j = cy - 1; j <= cy + 1; j++)
                if (i == cx && j == cy) {
                    place_spawns( GROUP_BREATHER_HUB_ONLY, 1, i, j, i, j, density );
                } else {
                    place_spawns( GROUP_BREATHER_ONLY, 1, i, j, i, j, density );
                }
        }
        break;
