...
building_gen_pointer get_mapgen_cfunction( const std::string &ident )
{
    static const std::map<std::string, building_gen_pointer> pointers = { {
            ...
            { "monorail_straight", &mapgen_monorail },
            { "monorail_curved",   &mapgen_monorail },
            { "monorail_end",      &mapgen_monorail },
            { "monorail_tee",      &mapgen_monorail },
            { "monorail_four_way", &mapgen_monorail },
...
void mapgen_monorail( mapgendata &dat )
{
    ...
    // start by filling the whole map with grass/dirt/etc
    dat.fill_groundcover();
    // which of the cardinal directions get monorails?
    bool monorails_nesw[4] = {};
    int num_dirs = terrain_type_to_nesw_array( dat.terrain_type(), monorails_nesw );
    // which way should our monorails curve, based on neighbor monorails?
    int curvedir_nesw[4] = {};
    for( int dir = 0; dir < 4; dir++ ) { // N E S W
        if( !monorails_nesw[dir] || dat.t_nesw[dir]->get_type_id().str() != "monorail" ) {
            continue;
        }
        // n_* contain details about the neighbor being considered
        bool n_monorails_nesw[4] = {};
        // TODO: figure out how to call this function without creating a new oter_id object
        int n_num_dirs = terrain_type_to_nesw_array( dat.t_nesw[dir], n_monorails_nesw );
        // if 2-way neighbor has a monorail facing us
        if( n_num_dirs == 2 && n_monorails_nesw[( dir + 2 ) % 4] ) {
            // curve towards the direction the neighbor turns
            if( n_monorails_nesw[( dir - 1 + 4 ) % 4] ) {
                curvedir_nesw[dir]--;    // our monorail curves counterclockwise
            }
            if( n_monorails_nesw[( dir + 1 ) % 4] ) {
                curvedir_nesw[dir]++;    // our monorail curves clockwise
            }
        }
    }
    // calculate how far to rotate the map so we can work with just one orientation
    // also keep track of diagonal monorails
    int rot = 0;
    bool diag = false;
    // TODO: reduce amount of logical/conditional constructs here
    switch( num_dirs ) {
        ...
        case 3: // tee
            ...
            break;                                       // N/E/W, don't rotate
        case 2: // straight or diagonal
            if( monorails_nesw[1] && monorails_nesw[3] ) {
                rot = 1;    // E/W, rotate  90 degrees
                break;
            }
            if( monorails_nesw[1] && monorails_nesw[2] ) {
                rot = 1;    // E/S, rotate  90 degrees
                diag = true;
                break;
            }
            if( monorails_nesw[2] && monorails_nesw[3] ) {
                rot = 2;    // S/W, rotate 180 degrees
                diag = true;
                break;
            }
            if( monorails_nesw[3] && monorails_nesw[0] ) {
                rot = 3;    // W/N, rotate 270 degrees
                diag = true;
                break;
            }
            if( monorails_nesw[0] && monorails_nesw[1] ) {
                diag = true;    // N/E, don't rotate
                break;
            }
            break;                                                                        // N/S, don't rotate
    ...
    // rotate the arrays left by rot steps
    nesw_array_rotate<bool>( monorails_nesw, 4, rot );
    nesw_array_rotate<int> ( curvedir_nesw,  4, rot );
    // now we have only these shapes: '   |   '-   -'-   -|-
    switch( num_dirs ) {
        case 4: // 4-way intersection
...
........................",
            ...
            break;
        case 2: // straight or diagonal
            if( diag ) { // diagonal monorails get drawn differently from all other types
                mapf::formatted_set_simple( m, 0, 0, "\
.^DD^^DD^.......^DD^^DD^\n\
..^DD^^DD^.......^DD^^DD\n\
...^DD^^DD^.......^DD^^D\n\
