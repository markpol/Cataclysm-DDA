...
tripoint rotate_point( const tripoint &p, int rotations )
{
    ...
    switch( rotations ) {
        ...
        case 1:
            ret.x = p.y;
            ret.y = SUBMAP_MAX_X - p.x;
            break;
        case 2:
            ret.x = SUBMAP_MAX_X - p.x;
            ret.y = SUBMAP_MAX_Y - p.y;
            break;
        case 3:
            ret.x = SUBMAP_MAX_Y - p.y;
            ret.y = p.x;
            break;
    }
...
void mapgen_crater(map *m, oter_id, mapgendata dat, int, float)
{
    ...
    m->place_items("wreckage", 83, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
}
...
void mapgen_field(map *m, oter_id, mapgendata dat, int turn, float)
{
    ...
    m->place_items("field", 60, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, turn); // fixme: take 'rock' out and add as regional biome setting
}
...
void mapgen_forest_general(map *m, oter_id terrain_type, mapgendata dat, int turn, float density )
{
    ...
    for (int i = 0; i <= SUBMAP_MAX_X; i++) {
        for (int j = 0; j <= SUBMAP_MAX_Y; j++) {
            ...
            if (SUBMAP_MAX_X - i < dat.e_fac) {
                forest_chance += dat.e_fac - (SUBMAP_MAX_X - i);
                num++;
            }
            if (SUBMAP_MAX_Y - j < dat.s_fac) {
                forest_chance += dat.s_fac - (SUBMAP_MAX_Y - j);
                num++;
            }
            if (i < dat.w_fac) {
    ...
    m->place_items("forest", 60, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, turn); // fixme: region settings
    ...
    if (terrain_type == "forest_water") {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            for (int j = 0; j < factor; j++) {
                int wx = rng(SEEX, SUBMAP_MAX_X), wy = rng(0, SUBMAP_MAX_Y);
                if (dat.is_groundcover( m->ter(wx, wy) ) ||
                      m->ter(wx, wy) == t_underbrush) {
            ...
            for (int j = 0; j < factor; j++) {
                int wx = rng(0, SUBMAP_MAX_X), wy = rng(SEEY, SUBMAP_MAX_Y);
                if (dat.is_groundcover( m->ter(wx, wy) ) ||
                      m->ter(wx, wy) == t_underbrush) {
                    m->ter_set(wx, wy, t_swater_sh);
                }
            ...
            for (int j = 0; j < factor; j++) {
                int wx = rng(0, SEEX - 1), wy = rng(0, SUBMAP_MAX_Y);
                if (dat.is_groundcover( m->ter(wx, wy) ) ||
                      m->ter(wx, wy) == t_underbrush) {
                    m->ter_set(wx, wy, t_water_sh);
                }
        ...
        for (int i = 0; i < rn; i++) {
            x = rng(0, SUBMAP_MAX_X);
            y = rng(0, SUBMAP_MAX_Y);
    ...
    if (one_in(10000)) {
        m->place_spawns( mongroup_id( "GROUP_JABBERWOCK_ONLY" ), 100, 0, 0, SEEX, SEEY, density ); //@todo: single spawn
    }

    //Very rare easter egg, ~1 per 10 overmaps
    if (one_in(1000000)) {
        m->place_spawns( mongroup_id( "GROUP_SHIA_ONLY" ), 100, 0, 0, SEEX, SEEY, density ); //@todo: single spawn
    }
    ...
    if (one_in(100)) {
        ...
        m->place_spawns( mongroup_id( "GROUP_SPIDER" ), rng(10, 20), 0, 0, SEEX, SEEY, density );
    }
}

void mapgen_hive(map *m, oter_id, mapgendata dat, int turn, float density )
{
    ...
    m->place_spawns( mongroup_id( "GROUP_BEE" ), 10, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, density );
    m->place_spawns( mongroup_id( "GROUP_BEEKEEPER" ), 20, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, density );
...
void mapgen_spider_pit(map *m, oter_id, mapgendata dat, int turn, float)
{
    ...
    for (int i = 0; i <= SUBMAP_MAX_X; i++) {
        for (int j = 0; j <= SUBMAP_MAX_Y; j++) {
            ...
            if (SUBMAP_MAX_Y - j < dat.s_fac) {
                forest_chance += dat.s_fac - (SUBMAP_MAX_X - j);
                num++;
            }
            if (i < dat.w_fac) {
                forest_chance += dat.w_fac - i;
...
void mapgen_fungal_bloom(map *m, oter_id, mapgendata dat, int, float density )
{
    (void)dat;
    for (int i = 0; i <= SUBMAP_MAX_X; i++) {
        for (int j = 0; j <= SUBMAP_MAX_Y; j++) {
    ...
    m->place_spawns( mongroup_id( "GROUP_FUNGI_QUEEN_ONLY" ), 100, SEEX, SEEY, SEEX, SEEY, density ); //@todo: single spawn
}

void mapgen_fungal_tower(map *m, oter_id, mapgendata dat, int, float density )
{
    (void)dat;
    for (int i = 0; i <= SUBMAP_MAX_X; i++) {
        for (int j = 0; j <= SUBMAP_MAX_Y; j++) {
    ...
    square(m, t_fungus, SEEX - 2, SEEY - 2, SEEX + 2, SEEY + 2);
    m->place_spawns( mongroup_id( "GROUP_FUNGI_TOWER_ONLY" ), 100, SEEX, SEEY, SEEX, SEEY, density );  //@todo: single spaw
}

void mapgen_fungal_flowers(map *m, oter_id, mapgendata dat, int, float density )
{
    (void)dat;
    for (int i = 0; i <= SUBMAP_MAX_X; i++) {
        for (int j = 0; j <= SUBMAP_MAX_Y; j++) {
    ...
    square(m, t_fungus, SEEX - 2, SEEY - 2, SEEX + 2, SEEY + 2);
    m->place_spawns( mongroup_id( "GROUP_FUNGI_SEEDER_ONLY" ), 100, SEEX, SEEY, SEEX, SEEY, density );  //@todo: single spaw
}
...
void mapgen_road( map *m, oter_id terrain_type, mapgendata dat, int turn, float density )
{
    ...
    if( diag ) { // diagonal roads get drawn differently from all other types
        ...
        for( int dir = 0; dir < 4; dir++ ) {
            if( roads_nesw[dir] ) {
                ...
                if( sidewalks_neswx[ ( dir + 1 ) % 4 ] ||  // has_sidewalk east?
                    ...
                    sidewalks_neswx[   dir           ] ) { // has_sidewalk north?
                    int x1 = SUBMAP_MAX_X - 4, y1 = 0, x2 = SUBMAP_MAX_X, y2 = SEEY - 1 + dead_end_extension;
                    coord_rotate_cw( x1, y1, dir );
                    coord_rotate_cw( x2, y2, dir );
                    square( m, t_sidewalk, x1, y1, x2, y2 );
                }
        ...
        if( dead_end_extension > 0 && sidewalks_neswx[ 2 ] ) {
            square( m, t_sidewalk, 0, SEEY + dead_end_extension, SUBMAP_MAX_X, SEEY + dead_end_extension + 4 );
        }
        ...
        for( int dir = 0; dir < 4; dir++ ) {
            if( roads_nesw[dir] ) {
                int x1 = 4, y1 = 0, x2 = SUBMAP_MAX_X - 4, y2 = SEEY - 1 + dead_end_extension;
                coord_rotate_cw( x1, y1, dir );
                coord_rotate_cw( x2, y2, dir );
                ...
                if( curvedir_nesw[dir] != 0 ) {
                    for( int x = 1; x < 4; x++ ) {
                        for( int y = 0; y < x; y++ ) {
                            int ty = y, tx = ( curvedir_nesw[dir] == -1 ? x : SUBMAP_MAX_X - x );
        ...
        if( plaza_dir > -1 ) {
            if( plaza_dir == 8 ) { // plaza center
                ...
                square( m, t_pavement, 0, SEEY - 10, SUBMAP_MAX_X, SEEY - 1 );
                square( m, t_sidewalk, 0, SEEY - 2 , SUBMAP_MAX_X, SUBMAP_MAX_Y );
                if( one_in( 3 ) ) {
                    line( m, t_tree_young, 1, SEEY, SUBMAP_MAX_X - 1, SEEY );
                }
                if( one_in( 3 ) ) {
                ...
                circle( m, t_pavement, 0, SUBMAP_MAX_Y, 21 );
                circle( m, t_sidewalk, 0, SUBMAP_MAX_Y, 13 );
                if( one_in( 3 ) ) {
                    circle( m, t_tree_young, 0, SUBMAP_MAX_Y, 11 );
    ...
    if( neighbor_sidewalks ) {
        m->place_spawns( mongroup_id( "GROUP_ZOMBIE" ), 2, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, density );
        // 1 per 10 overmaps
        if( one_in( 10000 ) ) {
    ...
    m->place_items( plaza ? "trash" : "road", 5, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, plaza,
                    plaza ? 0 : turn );
...
void mapgen_sewer_straight(map *m, oter_id terrain_type, mapgendata dat, int, float)
{
    (void)dat;
        ...
        m->place_items("sewer", 10, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
...
void mapgen_sewer_curved(map *m, oter_id terrain_type, mapgendata dat, int, float)
{
    (void)dat;
        ...
        m->place_items("sewer", 18, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
...
void mapgen_sewer_tee(map *m, oter_id terrain_type, mapgendata dat, int, float)
{
    (void)dat;
        ...
        m->place_items("sewer", 23, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
...
void mapgen_sewer_four_way(map *m, oter_id, mapgendata dat, int, float)
{
    (void)dat;
        ...
        m->place_items("sewer", 28, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
}
...
void mapgen_bridge(map *m, oter_id terrain_type, mapgendata dat, int turn, float)
{
    ...
    m->place_items("road", 5, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, false, turn);
}
...
void mapgen_highway(map *m, oter_id terrain_type, mapgendata dat, int turn, float)
{
    ...
    m->place_items("road", 8, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, false, turn);
}
...
void mapgen_generic_house(map *m, oter_id terrain_type, mapgendata dat, int turn, float density, int variant)
{
    ...
    rw = SUBMAP_MAX_X + 1 -rng(1, 5); // East external wall
    tw = rng(1, 6);  // North external wall
    bw = SUBMAP_MAX_X + 1 -rng(2, 5); // South external wall
    ...
    case 1: // Quadrants, essentially
        ...
        if (!one_in(3) && rw < SUBMAP_MAX_X) { // Potential side windows
            rn = rng(tw + 2, bw - 6);
    ...
    case 3: // Long center hallway, kitchen, living room and office
        ...
        if (one_in(3) && rw < SUBMAP_MAX_X) { // Side windows?
            rn = rng(tw + 1, cw - 2);
            m->ter_set(rw, rn    , t_window_domestic);
            m->ter_set(rw, rn + 1, t_window_domestic);
        }
    ...
    if (one_in(100)) { // todo: region data // Houses have a 1 in 100 chance of wasps!
        ...
        for (int i = 0; i < num_pods; i++) {
            ...
            m->place_spawns( mongroup_id( "GROUP_WASP" ), 10, podx, pody, podx, pody, density );
        }
        m->place_items("rare", 70, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, false, turn);
        ...
        for (int i = 0; i <= SUBMAP_MAX_X; i++) {
            for (int j = 0; j <= SUBMAP_MAX_Y; j++) {
                if (m->ter(i, j) == t_floor) {
                    if (one_in(15)) {
                        m->place_spawns( mongroup_id( "GROUP_SPIDER_GIANT" ), rng(10, 20), 0, 0, i, j, density );
        ...
        m->place_items("rare", 60, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, false, turn);

    } else { // Just boring old zombies
        m->place_spawns( mongroup_id( "GROUP_ZOMBIE" ), 20, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, density );
    }
...
void mapgen_pharm(map *m, oter_id terrain_type, mapgendata dat, int, float density) {
    ...
    int cw = 0;
        ...
        autorotate(false);
        m->place_spawns( mongroup_id( "GROUP_PHARM" ), 20, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, density );
    ...
    for (int i = 1; i < 22; i++) {
        for (int j = 1; j < 22; j++) {
            ...
            if( one_in( 30 ) && m->passable( i, j ) ) {
                m->furn_set( i, j, egg_type );
                m->place_spawns( mongroup_id( "GROUP_SPIDER_GIANT" ), rng(10, 20), i, j, i, j, density ); //hope you like'em spiders
                m->remove_field( { i, j, m->get_abs_sub().z }, fd_web );
            }
        }
    }
...
//    } else if (is_ot_type("police", terrain_type)) {
        ...
        for (int i = 0; i <= SUBMAP_MAX_X; i++) {
            for (int j = 0; j <= SUBMAP_MAX_Y; j++) {
                if ((j ==  7 && i != 17 && i != 18) ||
                    ...
                    (j == 15 && i > 17  && i < SUBMAP_MAX_X) ||
                    (j == 17 && i >  0  && i < 17) ||
                    (j == 20)) {
                    m->ter_set(i, j, t_wall);
                } else if (((i == 0 || i == SUBMAP_MAX_X) && j > 7 && j < 20) ||
        ...
        for (int i = 18; i < SUBMAP_MAX_X; i++) {
            m->ter_set(i, 20, t_window);
        }
        if (one_in(3)) {
            for (int j = 16; j < 20; j++) {
                m->ter_set(SUBMAP_MAX_X, j, t_window);
            }
        }
        int rn = rng(18, 21);
        if (one_in(4)) {
        ...
        m->place_spawns( mongroup_id( "GROUP_POLICE" ), 20, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, density );
...
void mapgen_cave(map *m, oter_id, mapgendata dat, int turn, float density)
{
        if (dat.above() == "cave") {
            ...
            case 1:
                ...
                m->place_items("cave_minerals", 50, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
                m->place_items("monparts", 80, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
                break;
            case 2:
                // trash, minerals less likely
                m->place_items("cave_minerals", 25, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
                m->place_items("trash", 70, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
                break;
            case 3:
                // bat corpses
                m->place_items("cave_minerals", 50, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
                for (int i = rng(1, SEEX); i > 0; i--) {
                    m->add_item_or_charges(rng(1, SUBMAP_MAX_X), rng(1, SUBMAP_MAX_Y), item::make_corpse( mon_bat ) );
                }
                break;
            case 4:
                // ant food, chance of 80
                m->place_items("cave_minerals", 25, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
                m->place_items("ant_food", 85, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
                break;
            case 5: {
                // hermitage
            ...
            m->place_spawns( mongroup_id( "GROUP_CAVE" ), 20, 6, 6, 18, 18, density );
...
void mapgen_cave_rat( map *m, oter_id, mapgendata dat, int, float density )
{
        ...
        if (dat.above() == "cave_rat") { // Finale
            ...
            for (int i = SEEX - 4; i <= SEEX + 4; i++) {
                for (int j = SEEY - 4; j <= SEEY + 4; j++) {
                    if ((i <= SEEX - 2 || i >= SEEX + 2) && (j <= SEEY - 2 || j >= SEEY + 2)) {
                        m->place_spawns( mongroup_id( "GROUP_SEWER_RAT" ), 10, i, j, i, j, density );
                    }
                }
            }
            m->place_spawns( mongroup_id( "GROUP_RAT_KING_ONLY" ), 100, 0, 0, SEEX, SEEY, density ); //@todo: single spawn
            ...
            do {
                ...
                for (int cx = cavex - 1; cx <= cavex + 1; cx++) {
                    for (int cy = cavey - 1; cy <= cavey + 1; cy++) {
                        ...
                        if (one_in(20)) {
                            m->place_spawns( mongroup_id( "GROUP_SEWER_RAT" ), 10, cx, cy, cx, cy, density );
                        }
                    }
                }
                if (cavey == SEEY - 1) {
                ...
                for (auto &i : path) {
                    for (int cx = i.x - 1; cx <= i.x + 1; cx++) {
                        for (int cy = i.y - 1; cy <= i.y + 1; cy++) {
                            ...
                            if (one_in(20)) {
                                m->place_spawns( mongroup_id( "GROUP_SEWER_RAT" ), 10, cx, cy, cx, cy, 1 );
                            }
                        }
                    }
                }
            ...
            m->ter_set(SEEX - 1, SUBMAP_MAX_X - 1, t_slope_up);
            m->ter_set(SEEX    , SUBMAP_MAX_X - 1, t_slope_up);
...
void mapgen_cavern(map *m, oter_id, mapgendata dat, int, float)
{
    ...
    m->place_items("cavern", 60, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, false, 0);
        ...
        for (int j = 0; j <= SUBMAP_MAX_Y; j++) {
            if ((dat.n_fac < 0 && j < dat.n_fac * -1) || (dat.s_fac < 0 && j >= SEEY * 2 - dat.s_fac) ||
                ...
                m->ter_set(i, j, t_rock_floor);
                       ...
                       i < dat.w_fac || i >= SUBMAP_MAX_X + 1 -dat.e_fac   ) {
                m->ter_set(i, j, t_rock);
            } else {
    ...
    switch (rng(0, 4)) { // Randomly chosen "altar" design
        ...
        case 1:
            for (int i = 6; i < 11; i++) {
                ...
                m->ter_set(SUBMAP_MAX_X - i, i, t_lava);
                m->ter_set(i, SUBMAP_MAX_Y - i, t_lava);
                m->ter_set(SUBMAP_MAX_X - i, SUBMAP_MAX_Y - i, t_lava);
                if (i < 10) {
                    m->ter_set(i + 1, i, t_lava);
                    m->ter_set(SUBMAP_MAX_X + 1 -i, i, t_lava);
                    m->ter_set(i + 1, SUBMAP_MAX_Y - i, t_lava);
                    m->ter_set(SUBMAP_MAX_X + 1 -i, SUBMAP_MAX_Y - i, t_lava);

                    m->ter_set(i, i + 1, t_lava);
                    m->ter_set(SUBMAP_MAX_X - i, i + 1, t_lava);
                    m->ter_set(i, SEEY * 2 - i, t_lava);
                    m->ter_set(SUBMAP_MAX_X - i, SEEY * 2 - i, t_lava);
                }
                if (i < 9) {
                    m->ter_set(i + 2, i, t_rock);
                    m->ter_set(SUBMAP_MAX_X + 1 -i + 1, i, t_rock);
                    m->ter_set(i + 2, SUBMAP_MAX_Y - i, t_rock);
                    m->ter_set(SUBMAP_MAX_X + 1 -i + 1, SUBMAP_MAX_Y - i, t_rock);

                    m->ter_set(i, i + 2, t_rock);
                    m->ter_set(SUBMAP_MAX_X - i, i + 2, t_rock);
                    m->ter_set(i, SEEY * 2 - i + 1, t_rock);
                    m->ter_set(SUBMAP_MAX_X - i, SEEY * 2 - i + 1, t_rock);
                }
            }
            break;
        case 2:
        ...
        case 3:
            for (int i = 6; i < 11; i++) {
                for (int j = 6; j < 11; j++) {
                    m->ter_set(i, j, t_lava);
                    m->ter_set(SUBMAP_MAX_X - i, j, t_lava);
                    m->ter_set(i, SUBMAP_MAX_Y - j, t_lava);
                    m->ter_set(SUBMAP_MAX_X - i, SUBMAP_MAX_Y - j, t_lava);
                }
            }
            break;
    }
...
void mapgen_ants_curved(map *m, oter_id terrain_type, mapgendata dat, int, float)
{
    ...
    for (int i = SEEX - 2; i <= SEEX + 3; i++) {
        ...
        m->ter_set(SUBMAP_MAX_X, i, t_rock_floor);
        m->ter_set(SUBMAP_MAX_X - 1, i, t_rock_floor);
        m->ter_set(SUBMAP_MAX_X - 2, i, t_rock_floor);
    }
    do {
        for (int i = x - 2; i <= x + 3; i++) {
            for (int j = y - 2; j <= y + 3; j++) {
                if (i > 0 && i < SUBMAP_MAX_X && j > 0 && j < SUBMAP_MAX_Y) {
                    m->ter_set(i, j, t_rock_floor);
                }
            }
        }
...
void mapgen_ants_four_way(map *m, oter_id, mapgendata dat, int, float)
{
    ...
    for (int i = 0; i <= SUBMAP_MAX_X; i++) {
        ...
        y += rng(-1, 1);
        while (abs(SEEY - y) > SUBMAP_MAX_X + 1 -i - 1) {
            if (y < SEEY) {
                y++;
            }
            if (y > SEEY) {
...
void mapgen_ants_straight(map *m, oter_id terrain_type, mapgendata dat, int, float)
{
    ...
    for (int j = 0; j <= SUBMAP_MAX_Y; j++) {
        ...
        x += rng(-1, 1);
        while (abs(SEEX - x) > SUBMAP_MAX_X + 1 -j - 1) {
            if (x < SEEX) {
                x++;
            }
            if (x > SEEX) {
...
void mapgen_ants_tee(map *m, oter_id terrain_type, mapgendata dat, int, float)
{
    ...
    for (int i = SEEX; i <= SUBMAP_MAX_X; i++) {
        ...
        y += rng(-1, 1);
        while (abs(SEEY - y) > SUBMAP_MAX_X - i) {
            if (y < SEEY) {
                y++;
            }
            if (y > SEEY) {
...
void mapgen_ants_generic(map *m, oter_id terrain_type, mapgendata dat, int, float)
{
    ...
    if (terrain_type == "ants_food") {
        m->place_items("ant_food", 92, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
    } else {
        m->place_items("ant_egg",  98, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
    }
    if (terrain_type == "ants_queen") {
        m->place_spawns( mongroup_id( "GROUP_ANT_QUEEN" ), 100, 0, 0, SEEX, SEEY, 1 ); //@todo: single spawn
...
void mapgen_ants_larvae(map *m, oter_id terrain_type, mapgendata dat, int turn, float density)
{
    mapgen_ants_generic(m, terrain_type, dat, turn, density);
    m->place_items("ant_egg",  98, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
    m->place_spawns( mongroup_id( "GROUP_ANT_LARVA" ), 10, 0, 0, SEEX, SEEY, density );
}
...
void mapgen_ants_queen(map *m, oter_id terrain_type, mapgendata dat, int turn, float density)
{
    mapgen_ants_generic(m, terrain_type, dat, turn, density);
    m->place_items("ant_egg",  98, 0, 0, SUBMAP_MAX_X, SUBMAP_MAX_Y, true, 0);
    m->place_spawns( mongroup_id( "GROUP_ANT_QUEEN" ), 100, 0, 0, SEEX, SEEY, density ); //@todo: single spawn
}
...
void mapgen_tutorial(map *m, oter_id terrain_type, mapgendata dat, int turn, float density)
{
    ...
    if( dat.zlevel < 0 ) {
        ...
        m->spawn_item(SUBMAP_MAX_X - 1, SEEY + 7, "cig");
        m->spawn_item(SUBMAP_MAX_X - 1, SEEY + 7, "codeine");
        m->spawn_item(SUBMAP_MAX_X - 2, SEEY + 7, "water");
