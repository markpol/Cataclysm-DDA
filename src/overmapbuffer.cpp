...
std::vector<city_reference> overmapbuffer::get_railroad_stations_near( const tripoint &location, int radius )
{
    std::vector<city_reference> result;
    ...
    for( const auto om : get_overmaps_near( location, radius ) ) {
        ...
        std::transform( om->railroad_stations.begin(), om->railroad_stations.end(), std::back_inserter( result ),
        [&]( city& element ) {
